 ...
 * search keys.
 * 
 * This code is based on HashedDictionaryOpenAddressingLinear which based on
 * HashedDictionary from Chapter 22 of Data Structures and Abstractions with
 * Java. 4/e by Carrano
 * 
 * It adds protocol that allows the client to change the load factor.
 * 
 * @version 4.1 (Created for the Solution) Instrumentation for counting the
 *          number of probes has been added
 * 
 * 
...
public class HashedDictionaryOpenAddressingDoubleInstrumented<K, V> implements DictionaryInterface<K, V> {
	...
	private int locate(int index, K key) {
		boolean found = false;
		int index2 = getSecondHashIndex(key);
		...
		while (!found && (hashTable[index] != null)) {
			if (hashTable[index].isIn() && key.equals(hashTable[index].getKey())) {
				...
				index = (index + i*index2) % hashTable.length; // Linear probing
				i++;
				totalProbes++;
			}
		} // end while
