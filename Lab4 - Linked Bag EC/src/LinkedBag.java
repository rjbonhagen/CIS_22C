...
// or null otherwise.
    ...
    public T remove() {
        ...
        if (numberOfEntries > 1) {
        	int randInd = (int)(numberOfEntries * (Math.random()));
            for (int i = 0; i < randInd && currNode != null; i++) {
    ...
    public boolean equals(LinkedBag<T> aBag) {
        ...
        if (numberOfEntries != aBag.numberOfEntries || aBag == null) {
        	return false;
        }
        if (numberOfEntries == 0 && aBag.numberOfEntries == 0) {
        	return true;
        }
        
        Node currNode = firstNode;
        while (currNode != null) {
        	if(getFrequencyOf(currNode.data) == aBag.getFrequencyOf(currNode.data)) {
        		result = true;
        	}
    ...
    public void removeDuplicates() {
        ...
        while (currNode != null) {
        	T entry = currNode.data;
        	int freq = getFrequencyOf(entry);
        	if (freq > 1) {
